...
if st.session_state.active_tab in ["PORTFÖYÜM", "PORTFÖY ANALİZİ"]:
    ...
    for sym, m_data in merged_holdings.items():
        ...
        if d:
            ...
            # TECHNICAL SCORE CALCULATION FOR PORTFOLIO
            t_score = 0.0; score_color = "rgba(255,255,255,0.1)"; score_label = "N/A"
            try:
                hist = get_history(sym, period="3mo", asset_type=t)
                if not hist.empty and len(hist) > 2:
                    st_series = calculate_supertrend(hist)
                    kama_series = calculate_kama(hist, period=min(21, len(hist)-2))
                    obv_series = calculate_obv(hist)
                    ...
                    st_above = d["price"] > l_st and l_st > 0
                    kama_above = d["price"] > l_kama and l_kama > 0
                    
                    # Score Components
                    if st_above: t_score += 3.0
                    if kama_above: t_score += 2.0
                    if st_above:
                        dist = ((d["price"] / l_st) - 1) * 100
                        if dist <= 4.0: t_score += 1.5
                        elif dist <= 12.0: t_score += 1.0
                        else: t_score += 0.4
                    if kama_above:
                        dist = ((d["price"] / l_kama) - 1) * 100
    ...
    if st.session_state.active_tab == "PORTFÖYÜM":
        ...
        with cols[1]:
            ...
            # Custom Header for All Assets (Centered balance)
            a_h_cols = st.columns([2.0, 1.6, 2.0, 1.8, 1.8, 1.8, 1.5, 1.5, 0.5])
            a_headers = ["VARLIK", "SKOR", "MALİYET", "TOPLAM ALIŞ", "GÜNCEL", "TOPLAM DEĞER", "GÜNLÜK K/Z", "TOPLAM K/Z", ""]
            for i, h_text in enumerate(a_headers):
                with a_h_cols[i]:
                    align = "center" if h_text == "SKOR" else "left"
            ...
            for i, h in enumerate(sorted_detailed_list):
                ...
                r_cols = st.columns([2.0, 1.6, 2.0, 1.8, 1.8, 1.8, 1.5, 1.5, 0.5])
                with r_cols[0]:
                    sig = h.get("Signal")
                ...
                with r_cols[1]: # SCORE COLUMN
                    st.markdown(f"""
                        <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; line-height:1; padding-top:5px;">
